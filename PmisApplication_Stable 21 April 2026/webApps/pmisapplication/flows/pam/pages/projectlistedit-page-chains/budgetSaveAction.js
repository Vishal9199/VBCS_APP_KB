  ...
  class budgetSaveAction extends ActionChain {
    ...
    async run(context) {
      ...
      try {
        ...
        // ================================================================
        const isEditMode = (budgetVar.budget_cost_id && budgetVar.budget_cost_id > 0 && $variables.pNavCode === 'EDIT');
        const httpMethod = isEditMode ? 'PUT' : 'POST';

        console.log('='.repeat(80));
        console.log('BUDGET COST LINE SAVE OPERATION');
        console.log('Mode:', isEditMode ? 'EDIT (PUT)' : 'CREATE (POST)');
        console.log('Budget Cost ID:', budgetVar.budget_cost_id);
        console.log('Object Version (Initial):', budgetVar.object_version_num);
        console.log('Project ID:', $variables.masterPlanHeaderVar.project_id);
        console.log('Budget Year ID:', $variables.masterPlanHeaderVar.budget_year_id);
        console.log('User:', $application.user.email);
        console.log('='.repeat(80));

        // ================================================================
        // STEP 3: FOR EDIT MODE - GET LATEST DATA TO ENSURE FRESH VERSION
        // ================================================================
        let latestObjectVersion = budgetVar.object_version_num;

        if (isEditMode) {
          console.log('🔄 EDIT mode - Fetching latest data from database...');
          
          try {
            // Encrypt the budget_cost_id for GET by ID
            const encryptedBudgetCostId = await Actions.callChain(context, {
              chain: 'application:encryptAC',
              params: {
            ...
            console.log('🔐 Budget Cost ID encrypted for GET request');

            // Call GET by ID endpoint to fetch latest data
            const getResponse = await Actions.callRest(context, {
              endpoint: 'PAM/getPmispamMasterplanLinebudgetcostGetbyid',
              headers: {
                'x-session-id': encryptedBudgetCostId,
              },
            });

            console.log('📥 GET by ID Response:', {
              status: getResponse.status,
              itemCount: getResponse.body?.items?.length || 0
            });

            // Check if data was retrieved successfully
            if (getResponse.status === 200 && getResponse.body?.items?.length > 0) {
              const latestData = getResponse.body.items[0];
              latestObjectVersion = latestData.object_version_num;
              
              console.log('✅ Latest data from database:');
              console.log('   object_version_num:', latestObjectVersion);
              console.log('📊 Version Comparison:');
              console.log('   Variable had:', budgetVar.object_version_num);
              console.log('   Database has:', latestObjectVersion);
              
              // ✅ Update the variable with latest version
              budgetVar.object_version_num = latestObjectVersion;
              
            } else {
              console.error('❌ GET by ID failed or returned no data');
              
              await Actions.fireNotificationEvent(context, {
                summary: 'Error',
                message: 'Failed to fetch latest record data. Please refresh the page.',
                displayMode: 'transient',
                type: 'error',
              });
              return;
            ...
            console.error('❌ Error fetching latest data:', getError);
            
            await Actions.fireNotificationEvent(context, {
              summary: 'Error',
              message: 'Failed to verify record version. Please refresh and try again.',
              displayMode: 'transient',
              type: 'error',
            });
            return;
        ...
        console.log('📝 Final object_version_num to be sent:', latestObjectVersion);
        ...
        const budgetPayload = {
          project_id: $variables.masterPlanHeaderVar.project_id,
          object_version_num: budgetVar.object_version_num,
        ...
        // ✅ ADD THESE FIELDS ONLY FOR EDIT MODE
        if (isEditMode) {
          budgetPayload.budget_cost_id = budgetVar.budget_cost_id;
          budgetPayload.object_version_num = latestObjectVersion; // ← Use fresh version from GET by ID
        }
        ...
        // ================================================================
        // STEP 8: CALL REST API ENDPOINT (POST/PUT)
        // ================================================================
        console.log('📤 Calling postPmispamBudgetlinecostProcess endpoint...');
        ...
        // ================================================================
        // STEP 10: SUCCESS - UPDATE LOCAL VARIABLES
        // ================================================================
        console.log('✅ Budget allocation saved successfully!');

        // ✅ CRITICAL: Update object_version_num from response
        if (response.body?.object_version_num) {
          $variables.budgetCostLineVar.object_version_num = response.body.object_version_num;
          console.log('📝 Object version updated to:', response.body.object_version_num);
        }

        // ✅ CRITICAL: If CREATE mode, store new IDs from response
        if (!isEditMode) {
          // Get encrypted ID from response header
          const newBudgetCostId = response.headers.get('X-Session-Id') 
            || response.headers.get('x-session-id') 
            || response.headers.get('X-session-id');
          
          if (newBudgetCostId) {
            console.log('🔑 New encrypted Budget Cost ID received:', newBudgetCostId);
          }
          
          // Get actual ID from response body (if backend returns it)
          if (response.body?.budget_cost_id) {
            $variables.budgetCostLineVar.budget_cost_id = response.body.budget_cost_id;
            console.log('🆔 Budget Cost ID set to:', response.body.budget_cost_id);
          }
        }

        // ================================================================
        // STEP 11: REFRESH DATA FROM DATABASE
        // ================================================================
        console.log('🔄 Refreshing budget cost data from database...');
        ...
        await Actions.fireNotificationEvent(context, {
          summary: 'Success',
          message: errorMsg || 'Budget allocation saved successfully',
          displayMode: 'transient',
          type: 'confirmation',
        });
