  ...
  class vbAfterNavigateListener extends ActionChain {
    async run(context) {
      ...
      try {
        ...
        const rawData = response.body.P_OUTPUT || [];

        // ====================================================================
        // ENRICH: fetch LOV tables in parallel to resolve display labels that
        // the search API does not return directly.
        // ====================================================================
        if (rawData.length > 0) {
          const [regionLovResp, projectLovResp, tenderLovResp] = await Promise.all([
            Actions.callRest(context, { endpoint: 'PmisSetup/getPmisLovRegionName' }),
            Actions.callRest(context, { endpoint: 'PDD/getPmispddLovTender' }),
            Actions.callRest(context, { endpoint: 'PDD/getPmispddLovTenderDtl' }),
          ]);

          // region_id (lookup_value_id) → region_name (lookup_value_name)
          const regionMap = {};
          (regionLovResp.body.items || []).forEach(r => {
            if (r.lookup_value_id != null) {
              regionMap[r.lookup_value_id] = r.lookup_value_name;
            }
          });

          // project_id → project_number
          const projectMap = {};
          (projectLovResp.body.items || []).forEach(p => {
            if (p.project_id != null) {
              projectMap[p.project_id] = p.project_number;
            }
          });

          // tender_id → tender_name
          const tenderMap = {};
          (tenderLovResp.body.items || []).forEach(t => {
            if (t.tender_id != null) {
              tenderMap[t.tender_id] = t.tender_name;
            }
          });

          $variables.regionADP.data = rawData.map(row => ({
            ...row,
            region_name:    regionMap[row.region_id]   || row.region_name    || '',
            project_number: projectMap[row.project_id] || row.project_number || '',
            tender_name:    tenderMap[row.tender_id]   || row.tender_name    || '',
          }));

          console.log("Region ADP enriched:", JSON.stringify($variables.regionADP.data));
        } else {
          $variables.regionADP.data = rawData;
        }
