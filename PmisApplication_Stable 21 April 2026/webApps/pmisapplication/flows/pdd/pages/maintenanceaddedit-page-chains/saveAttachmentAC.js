define([
  ...
  class saveAttachmentDialogBtnAC extends ActionChain {
    ...
    async run(context, { event, originalEvent }) {
      ...
      try {
        // ========== MAP TRANSACTION ID ==========
        // CREATE mode → use pNavEncId (same as reference chain)
        // EDIT mode   → use maintenance_id from saved record
        if ($variables.pNavCode === 'CREATE') {
          $variables.maintenanceadminSearchVar.maintenance_id = $variables.pNavEncId;
          encryptedTransactionId = $variables.pNavEncId;
        } else {
          encryptedTransactionId = $variables.maintenanceadminSearchVar.maintenance_id;
        }
        ...
        // ========== CONSTANTS ==========
        const docCategory = 'PMIS_MAINTENANCE';
        ...
        console.log('📥 Attachment Response:', response.body);
