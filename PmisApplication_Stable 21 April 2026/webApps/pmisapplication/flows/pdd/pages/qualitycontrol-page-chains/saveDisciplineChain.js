define([
  ...
  class saveDisciplineChain extends ActionChain {
    async run(context) {
      ...
      try {
        ...
        const submittableItems = await $page.variables.discBufferDP.instance.getSubmittableItems();
        ...
        console.log('Discipline: saving ' + updatedItems.length + ' changed record(s)');
        ...
        for (let i = 0; i < updatedItems.length; i++) {
          ...
          console.log('Saving record ' + (i + 1) + ', ID: ' + record.discipline_id);

          try {
            const encryptedId = await Actions.callChain(context, {
              chain: 'application:encryptAC',
              params: { input: String(record.discipline_id) },
            });
            ...
            const saveResponse = await Actions.callRest(context, {
              endpoint: 'PDD/postPmispddQualitycontrolDisciplineProcess',
            ...
            if (saveResponse.ok) {
              ...
              $page.variables.discBufferDP.instance.setItemStatus(updatedItems[i], 'submitted');
              successCount++;
            } else {
        ...
        console.log('Discipline reloaded:', JSON.stringify(dataResponse.body.OUTPUT));

        // CORRECT VBCS pattern: direct assignment to ADP.data
        $variables.disciplineADP.data = dataResponse.body.OUTPUT || [];

        // Reset BufferingDP after reload
        await $page.variables.discBufferDP.instance.resetAllUnsubmittedItems();

        if (failCount === 0) {
          await Actions.fireNotificationEvent(context, {
            summary: 'All Discipline records saved successfully (' + successCount + ' records)',
            displayMode: 'transient',
            type: 'confirmation',
          });
        } else {
        ...
        await Actions.fireNotificationEvent(context, {
          summary: 'Failed to save Discipline data',
          displayMode: 'persist',
          type: 'error',
        });
      } finally {
