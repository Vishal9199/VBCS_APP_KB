define([
  ...
  class saveContinualImproveChain extends ActionChain {
    async run(context) {
      ...
      try {
        ...
        const submittableItems = await $page.variables.ciBufferDP.instance.getSubmittableItems();
        ...
        console.log('Continual Improvement: saving ' + updatedItems.length + ' changed record(s)');
        ...
        for (let i = 0; i < updatedItems.length; i++) {
          ...
          console.log('Saving record ' + (i + 1) + ', ID: ' + record.continual_improve_id);

          try {
            const encryptedId = await Actions.callChain(context, {
              chain: 'application:encryptAC',
              params: { input: String(record.continual_improve_id) },
            });
            ...
            const saveResponse = await Actions.callRest(context, {
              endpoint: 'PDD/postPmispddQualitycontrolContinualimproveProcess',
            ...
            if (saveResponse.ok) {
              ...
              $page.variables.ciBufferDP.instance.setItemStatus(updatedItems[i], 'submitted');
              successCount++;
            } else {
        ...
        const encryptedType = await Actions.callChain(context, {
          chain: 'application:encryptAC',
          params: { input: 'CONTINUAL_IMPROVE' },
        });
        ...
        console.log('Continual Improvement reloaded:', JSON.stringify(dataResponse.body.OUTPUT));
        ...
        await $page.variables.ciBufferDP.instance.resetAllUnsubmittedItems();

        if (failCount === 0) {
          await Actions.fireNotificationEvent(context, {
            summary: 'All Continual Improvement records saved successfully (' + successCount + ' records)',
            displayMode: 'transient',
            type: 'confirmation',
          });
        } else {
        ...
        console.error('Error saving Continual Improvement data:', error);
        await Actions.fireNotificationEvent(context, {
          summary: 'Failed to save Continual Improvement data',
          displayMode: 'persist',
          type: 'error',
        });
      } finally {
  ...
  return saveContinualImproveChain;
});