  ...
  class lookupValueAddEditAC extends ActionChain {
    ...
    async run(context, { method }) {
      ...
      if ($variables.isValueFormValid === "valid") {
        ...
        const headerId = await $application.functions.encryptJs($application.constants.secretKey, $variables.lookupValueVar.lookup_value_id);
        ...
        $variables.encryptedBody.payload = await $application.functions.encryptJs($application.constants.secretKey, $variables.lookupValueVar);


        const response = await Actions.callRest(context, {
          endpoint: 'Look_Up/postLookupValueProcess',
          headers: {
