        ...
        if (event.value) {
          if (event.value.$tag === "_root_") {
            ...
            data.criteria.forEach(criteria => {
              ...
              if (criteria.$tag === "ScheduleNameFilter" && criteria.value && criteria.value.schedule_name) {
                l_menu_id_from_name_filter = criteria.value.schedule_name;
              }
            ...
            if (data.$tag === "ScheduleNameFilter" && data.value && data.value.schedule_name) {
              l_menu_id_from_name_filter = data.value.schedule_name;
            }
          }

          // Assign to search object
          if (l_menu_id_from_name_filter) {
            $variables.searchObj.SCHEDULE_ID= +l_menu_id_from_name_filter;
          }
