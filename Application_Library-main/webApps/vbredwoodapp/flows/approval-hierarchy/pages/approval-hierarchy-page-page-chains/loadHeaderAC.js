define([
  ...
  class loadHeaderAC extends ActionChain {
    ...
    async run(context) {
      ...
      $variables.searchObj.P_HIERARCHY_ID = $variables.key;
      ...
      const response = await Actions.callRest(context, {
        endpoint: 'approvalHeirarchy/postApprovalHierarchySearch',
        body: $variables.encryptedBody,
      });

      if (response?.body?.OUT_COUNT === 1) {
        $variables.approvalHierarchyVar = response.body.P_OUTPUT[0];
