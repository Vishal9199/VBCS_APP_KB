    ...
    async run(context) {
      ...
      if ($variables.isFormValid === "valid") {
        ...
        if ($variables.op === "POST") {
          $variables.lovConfigurationVar.created_by = $application.user.fullName;
          $variables.lovConfigurationVar.created_date = await $application.functions.getSysdate();
        }
        $variables.lovConfigurationVar.last_updated_by = $application.user.fullName;
        $variables.lovConfigurationVar.last_updated_date = await $application.functions.getSysdate();
        $variables.lovConfigurationVar.last_updated_login = $application.user.fullName;


        $variables.encryptedBody.payload = await $application.functions.encryptJs($application.constants.secretKey, $variables.lovConfigurationVar);

        // 🌐 Call REST
        response = await Actions.callRest(context, {
          endpoint: 'Program/postProgramLovProcess',
          headers: {
        ...
        const vg = document.getElementById("lov-validation");
