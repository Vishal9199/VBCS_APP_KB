  ...
  class saveMasterPlanAction extends ActionChain {
    ...
    async updateMasterPlan(context) {
      ...
      if (response.body.P_ERR_CODE === 'S') {
        ...
        // Save budget cost line data if budget_year_id exists
        if ($variables.masterPlanHeaderVar.budget_year_id) {
          try {
            console.log("💰 Saving budget cost line data...");
            await Actions.callChain(context, {
              chain: 'budgetSaveAction',
            });
            console.log("✅ Budget data saved successfully");
          } catch (budgetError) {
            console.error("⚠️ Error saving budget data:", budgetError);
            // Continue even if budget save fails
          }
        }
        ...
        await Actions.fireNotificationEvent(context, {
          summary: 'Success',
          message: 'Master Plan and Budget data saved successfully',
          type: 'confirmation',
          displayMode: 'transient',
        });
