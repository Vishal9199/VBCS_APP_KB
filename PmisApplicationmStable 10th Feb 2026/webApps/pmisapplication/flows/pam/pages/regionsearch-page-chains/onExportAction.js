define([
  ...
  class onExportRegionAction extends ActionChain {

    /**
     * Export all region records to Excel with professional styling
    ...
    async run(context) {
      ...
      try {
        console.log("📊 Starting Region Excel export with ExcelJS styling...");
        ...
        // ✅ CRITICAL FIX: Use exact same pattern as working code
        ...
        workbook.creator = 'VBCS Region Export';
        ...
        const worksheet = workbook.addWorksheet('Region Details', {
          properties: {
            tabColor: { argb: 'FF01474d' },
            defaultRowHeight: 20
          },
          pageSetup: {
            paperSize: 9, // A4
        ...
        console.log("📋 Configuring columns for Region Details...");
        ...
        worksheet.columns = [
          ...
          { header: 'Tender Name', key: 'tender_name', width: 30 },
          { header: 'Responsible Person', key: 'responsible_person', width: 25 },
          ...
          { header: 'Last Update Login', key: 'last_update_login', width: 20 }
        ];
        ...
        allRecords.forEach((record, index) => {
          try {
            ...
            const rowData = {
              serial_no: index + 1,
              region_id: this.sanitizeValue(record.region_id),
              region_name: this.sanitizeValue(record.region_name),
              project_id: this.sanitizeValue(record.project_id),
              project_number: this.sanitizeValue(record.project_number),
              tender_number: this.sanitizeValue(record.tender_number),
              tender_name: this.sanitizeValue(record.tender_name),
              responsible_person: this.sanitizeValue(record.responsible_person),
              person_id: this.sanitizeValue(record.person_id),
            ...
            row.getCell(2).alignment = { vertical: 'middle', horizontal: 'center' }; // Region ID
            row.getCell(4).alignment = { vertical: 'middle', horizontal: 'center' }; // Project ID
            row.getCell(9).alignment = { vertical: 'middle', horizontal: 'center' }; // Person ID
        ...
        const filename = `RegionDetails_${day}_${month}_${year}.xlsx`;
        ...
        await Actions.fireNotificationEvent(context, {
          summary: `Successfully exported region records`,
