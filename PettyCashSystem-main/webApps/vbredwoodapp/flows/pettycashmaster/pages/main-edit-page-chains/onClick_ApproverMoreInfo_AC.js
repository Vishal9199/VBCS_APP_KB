define([
  ...
  class onClick_ApproverMoreInfo_AC extends ActionChain {
    ...
    async run(context, { event, originalEvent }) {
      ...
      if ($flow.variables.actionValidation.isRpaResponseFormValid === "valid") {
        try {
          ...
          // ============================================
          // STEP 1: Package Update (RPA_RESPONSE)
          // ============================================
          console.log('Step 1: Updating action with RPA_RESPONSE...');
          ...
          if (updateResponse?.body?.P_ERROR_CODE !== "S") {
            throw new Error(updateResponse?.body?.P_ERROR_MSG || 'RPA_RESPONSE action update failed');
          }
          ...
          const terminateBody = {
            action: 'TERMINATE',
            instanceActionProps: {
              reason: $flow.variables.responseComments || 'Terminated for RPA_RESPONSE resubmission',
            },
          };
          ...
          const submissionType = $variables.payload.status_code === 'RIA' ? 'MORE_INFO' : 'INITIAL';
          ...
          await Actions.fireNotificationEvent(context, {
            summary: 'RPA Response Completed Successfully',
        ...
        // ❌ Validation failed
        await $functions.handleValidationError($flow.variables.actionValidation.rpaResponseValidation);
      }
    }
  }

  return onClick_ApproverMoreInfo_AC;
});