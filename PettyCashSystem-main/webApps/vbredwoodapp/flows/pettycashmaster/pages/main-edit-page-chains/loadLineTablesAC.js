define([
  ...
  class loadLineTablesAC extends ActionChain {
     ...
     * @param {Object} params
     * @param {string} params.key_param Input Parameter
     */
    async run(context, { key_param }) {
      const { $page, $flow, $application, $constants, $variables } = context;

      let temp_key_param = Number(key_param);
      ...
      const results = await Promise.all([
        async () => {
          ...
          let tempPayload = {
            ...
            'p_keyword': '',
            'p_claim_line_id': '',
            'p_claim_header_id': temp_key_param,
          ...
          $variables.totalRecords_Claim = response.body.OUT_TOTAL_COUNT;
          ...
          console.log("📊 Records on page:", $variables.claimLineADP.data.length);
          console.log("📈 Total records:", $variables.totalRecords_Claim);
          ...
          if (response.body.P_OUTPUT && response.body.P_OUTPUT.length > 0) {
            ...
            // $variables.payload.claim_amt = totalClaim;
            } 
            else {
        ...
        async () => {
          ...
          $variables.totalRecords_Receipts = response2.body.OUT_TOTAL_COUNT;

          // ✅ CALCULATE CURRENT PAGE
          const offset = parseInt($variables.SearchObj_ReceiptLine.in_offset, 10);
          const limit = parseInt($variables.SearchObj_ReceiptLine.in_limit, 10);
          $variables.currentPage = Math.floor(offset / limit) + 1;
      
          console.log("📄 Current Page:", $variables.currentPage);
          console.log("📊 Records on page:", $variables.claimLineADP.data.length);
          console.log("📈 Total records:", $variables.totalRecords_Receipts);
